# Test workflow for Limit Wait Time
...
# WORKFLOW
resource "n8n_workflow" "test_limit-wait-time" {
  name   = "Test: Limit Wait Time"
  active = false
...
# OUTPUTS
output "workflow_id" {
  value       = n8n_workflow.test_limit-wait-time.id
  description = "ID of the test workflow"
}

output "workflow_name" {
  value       = n8n_workflow.test_limit-wait-time.name
